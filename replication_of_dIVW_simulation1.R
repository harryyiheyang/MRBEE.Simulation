...
# MVMR median
mr_median_obj <- mr_mvmedian(mr_mvinput(bx = beta.exposure,bxse = se.exposure,by = beta.outcome,byse = se.outcome),iterations = 100)
...
bxse <- se.exposure
byse <- se.outcome
fit=MRBEE.IMRP(by=by,bX=bx,byse=byse,bXse=bxse,Rxy=R) # stores causal estimates and some model characteristics
...
if(i %% 10 ==0 ){
par(mfrow=c(3,1))
boxplot(res.beta1[1:i,seq(1,13,2)])
lines(c(1:7),rep(-0.5,7))
boxplot(res.beta2[1:i,seq(1,13,2)])
lines(c(1:7),rep(-0.7,7))
boxplot(res.beta3[1:i,seq(1,13,2)])
lines(c(1:7),rep(0.3,7))
}
}
A=list(beta1=res.beta1,beta2=res.beta2,beta3=res.beta3)
ResultList[[m]]=A
